...
class USTC4k_tracklets_0111(object):
    """
    USTC4kVidReID
    ...
    def __init__(self, root, min_seq_len=0, verbose=True, **kwargs):
        ...
        self.min_seq_len_train = 60
        self.dataset_dir = osp.join(root, 'ustc4k-tracklets-1231')
        self.test_dir = osp.join(root, 'ustc4k-181213')

        self.split_train_json_path = osp.join(self.dataset_dir, '0111_split_train.json')
        self.split_valquery_json_path = osp.join(self.dataset_dir, '0111_split_valquery.json')
        self.split_valgallery_json_path = osp.join(self.dataset_dir, '0111_split_valgallery.json')
    ...
    def _process_dir_tracklet(self):
        ...
        for i, cam_data in enumerate(self.cam_relabel):
            for pid, tracklets in cam_data.items():
                if pid in self.pids_train:
                    pid_ori = tracklets[0][2]
                    tdir = osp.join(self.dataset_dir, 'cam%d'%(i+1), '%04d'%pid_ori)
                    img_paths = sorted(glob.glob(osp.join(tdir, '*.jpg')))
                    img_paths = random.sample(img_paths, self.num_per_person)    #
                    img_paths = tuple(img_paths)
